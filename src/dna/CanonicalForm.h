...
{ switch ( form )
  ...
  case CanonicalForm::FWD:
    form = CanonicalForm::REV;
    break;
  case CanonicalForm::REV:
    form = CanonicalForm::FWD;
    break;
...
public:
  ...
  template <class Itr>
  static bool isFwd( Itr beg ) {
    return getForm(beg) == CanonicalForm::FWD;
  }

  template <class Itr>
  static bool isRev( Itr beg ) {
    return getForm(beg) == CanonicalForm::REV;
  }
