...
public:
  ...
  size_t getAmbiguityCount() const {
    return mEndBases-mBases;
  }
  ...
  unsigned char random( RNGen& rnGen ) const;
...
private:
  ...
  unsigned char chooseRandom( RNGen& rnGen ) const;

  static void fromBitsFatalErr( unsigned char bits )
  __attribute__((__noreturn__));
  static void fromCharFatalErr( char chr ) __attribute__((__noreturn__));
...
inline unsigned char GeneralizedBase::random( RNGen& rnGen ) const
{
  return mEndBases == mBases+1 ? Base::char2Val(*mBases) : chooseRandom(rnGen);
}
