...
{
  clearCache();
  Destroy(_ID1),Destroy(_ID2),Destroy(_lib_IDs),Destroy(_libs);
  _n_reads = _n_pairs = 0;
}
                            ...
                            const bool grow_read_count)
{
  ...
  ForceAssertGe( ID2, 0 );
  if (grow_read_count) {
    _n_reads = std::max( {ID1 + 1, ID2 + 1, _n_reads} );
  } else {
...
int PairsManager::addLibrary( const int & sep, const int & sd, const String& name)
{
...
bool PairsManager::isInterleaved( ) const
{
