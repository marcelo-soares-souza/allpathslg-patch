...
public:
  ...
  /// copy constructor
  File( File const& file ) : mType(UNKNOWN), mpStat(0) {
    *this = file;
  }
  ...
  { AssertEq(newExtension.find_first_of('/'),std::string::npos);
    ...
    std::string newPath;
    newPath.reserve(mPath.size()+1+xSize);
    newPath = mPath;
    if ( xSize )
