...
public:
  ...
  void setNReads( longlong const nReads )
  {
    AssertLe(2*_n_pairs,nReads);
    _n_reads = nReads;
  }
  ...
  // Add a new library, returning the new library ID
  int addLibrary( const int & sep, const int & sd, const String& lib_name = "");
  ...
  longlong ID1( const longlong & pair_ID ) const {
    return _ID1[pair_ID];
  }
  longlong ID2( const longlong & pair_ID ) const {
    return _ID2[pair_ID];
  }
  ...
  int libraryID( const longlong & pair_ID ) const {
    return _lib_IDs[pair_ID];
  }
  String libraryName( const longlong & pair_ID ) const {
    return _libs[ _lib_IDs[pair_ID] ]._name;
  }
  ...
  void AddSeps(const vec<int>& seps_) {
    seps = seps_;
  }
  void AddSDs(const vec<int>& sds_) {
    sds = sds_;
  }
  int sep2( const longlong & pair_ID ) const {
    return seps[pair_ID];
  }
  int sd2 ( const longlong & pair_ID ) const {
    return sds[pair_ID];
  }
  ...
  bool isPaired  ( const longlong & read_ID ) const {
    return ( getPairID(read_ID) != -1 );
  }
  bool isUnpaired( const longlong & read_ID ) const {
    return ( getPairID(read_ID) == -1 );
  }
  ...
  String getLibraryName( const int & i ) const {
    return _libs[i]._name;
  }
  int getLibrarySep( const int & i ) const {
    return _libs[i]._sep;
  }
  int getLibrarySD ( const int & i ) const {
    return _libs[i]._sd;
  }
  void setLibrarySep( const int & i, int sep ) {
    _libs[i]._sep = sep;
  }
  void setLibrarySD ( const int & i, int sd ) {
    _libs[i]._sd = sd;
  }
  ...
  // Determine if the reads in the assoicated fastb file are interleaved
...
private:
  ...
  void clearCache( ) const {
    Destroy(_pairs_index);
    Destroy(_partners_index);
  }

public:
  void makeCache( ) const {
    makePairsIndexCache( );
    makePartnersIndexCache( );
  }
