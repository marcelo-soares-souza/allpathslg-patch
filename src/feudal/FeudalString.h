 ...
 * \date Oct 1, 2009
 *
 * \brief Exactly like std::string, but only 2^32 elements, uses SerfVec for containment
 */
...
public:
  ...
  typedef SerfVec<charT> container;
  typedef typename SerfVec<charT>::alloc_type Allocator;
  ...
  FeudalString(FeudalString&& that)
    : mContainer(std::move(that.mContainer)) {}
  ...
  {
    if ( this != &that ) assign(that.begin(),that.end());
    return *this;
  }
  FeudalString& operator=(FeudalString&& that)
  {
    mContainer = std::move(that.mContainer);
    return *this;
  }
  ...
  {
    memcpy(dest,data()+pos,lenLimit(pos,n));
    return n;
  }
  ...
  {
    mContainer.swap(that.mContainer);
    return *this;
  }
  ...
  // Replace by ReverseComplement (assumes string consists of ACGT).

  void ReverseComplement( )
  { for ( size_t i = 0; i < size( )/2; i++ )
    { unsigned char c = (*this)[ size( ) - i - 1 ];
      (*this)[ size( ) - i - 1 ] = (*this)[i];
      (*this)[i] = c;
    }
    for ( size_t i = 0; i < size( ); i++ )
    { if ( (*this)[i] == 'A' ) (*this)[i] = 'T';
      else if ( (*this)[i] == 'C' ) (*this)[i] = 'G';
      else if ( (*this)[i] == 'G' ) (*this)[i] = 'C';
      else if ( (*this)[i] == 'T' ) (*this)[i] = 'A';
      else
      { std::cout << "Illegal attempt to reverse complement String."
                  << std::endl;
        ForceAssert( 0 == 1 );
      }
    }
  ...
  {
    reserve(size()+n);
    mContainer.insert(begin(pos1),n,c);
    return *this;
  }
  ...
  { using std::distance;
    reserve(size()+distance(first,last));
    mContainer.insert(begin(pos1), first, last);
    return *this;
  }
  template <class InputIterator>
  ...
  {
    erase(begin(pos),begin(pos+lenLimit(pos,n)));
    return *this;
  }
  iterator erase(iterator p) {
    return mContainer.erase(p);
  }
...
{ for ( ; i1 != i2 && n2; ++i1, --n2 ) {
  ...
  /// note: the empty string is considered a 0-valued int for reasons that are
  ...
  /// note: the empty string is considered a 0.-valued double for reasons that
  ...
  {
    erase(0,find_first_not_of(cstr));
    return *this;
  }
  ...
  {
    resize(find_last_not_of(cstr)+1);  // npos trick
    return *this;
  }
...
private:
  ...
  { AssertLe(pos,size());
    using std::min;
    return min(len,size()-min(size(),pos));
  }
