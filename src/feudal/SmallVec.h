...
public:
  ...
  { size_type offset = (&*position) - data();
    ...
    while((end - offset) >= n)
    {
      swap_back_group(data() + end, n);
      end -= n;
    }
    while(end != offset)
    {
      swap_back_single(data() + end, n);
      --end;
    }
  }
  ...
  { const_pointer itr1 = v1.data();
    ...
    using std::min;
    const_pointer end = itr1 + min(v1.size(),v2.size());
    int result = 0;
  ...
  { return BinaryReader::externalSizeof(static_cast<T*>(nullptr)) ?
           ...
           sizeof(size_type);
  } // for efficiency with complex T's
