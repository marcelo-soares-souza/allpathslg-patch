...
public:
  ...
  reverse_iterator rbegin() {
    return reverse_iterator(0,dataEnd());
  }
  ...
  reverse_iterator rend() {
    return reverse_iterator(size(),dataEnd());
  }
  ...
  { size_t result = 0;
    ...
    { size_t eleSiz = ele.size();
      if ( eleSiz >= K ) result += eleSiz-K+1;
    }
    return result;
  }
  ...
  size_type capacity() const {
    return mpAllocEnd - mpElements;
  }
  bool empty() const {
    return mpCurEnd == mpElements;
  }
  bool full() const {
    return mpCurEnd == mpAllocEnd;
  }

  OuterVec& reserve( size_type sz )
  {
    if ( sz > capacity() ) realloc(sz);
    return *this;
  }
  ...
  OuterVec& append( size_type sz,
                    ...
                    size_type cap = 0 )
  { using std::max;
    size_type reserveSize = max(size()+sz,cap);
  ...
  { using std::swap;
    ...
    { if ( begin != end && static_cast<size_type>(*begin) == iii )
      {
        ++begin;
        Assert(begin==end||static_cast<size_type>(*begin)>iii);
      }
      else
      {
        if ( iii != dest ) swap(elements[iii],elements[dest]);
        ++dest;
      }
    }
  ...
  OuterVec& clear() {
    mpCurEnd = destroy(data());
    return *this;
  }
  OuterVec& shrink_to_fit() {
    if ( !full() ) realloc(size());
    return *this;
  }
  ...
  { value_type* itr1 = v1.data();
    ...
    using std::min;
    value_type* end = itr1 + min(v1.size(),v2.size());
    int result = 0;
...
private:
  ...
  T const* dataEnd() const {
    return mpCurEnd;
  }

  bool isSelf( T const& val ) {
    return data() <= &val && &val < dataEnd();
  }
  ...
  { T* dest = dataEnd();
    ...
    {
      size_t nnn = itr->allocSize();
      if ( nnn <= maxVs ) nVs += nnn;
    }
  ...
  { using std::max;
    size_type reserveSize = max(sz,cap);
    if ( reserveSize > capacity() && isSelf(exemplar) )
    {
