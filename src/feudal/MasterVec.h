 ...
 * Must have static size_type interpretSize( void* pFixed, size_t varDataLen );
 */
template <class T>
...
public:
  ...
  {
    BaseT::push_back(val,growthFact,growthIncr);
    return *this;
  }
  ...
  {
    BaseT::append(that.begin(),that.end());
    return *this;
  }
  ...
  {
    BaseT::append(that.begin(from),that.begin(to));
    return *this;
  }

  MasterVec const& WriteAll( String const& fileName ) const
  {
    return WriteRange(fileName,0UL,BaseT::size());
    return *this;
  }
  ...
  { VirtualMasterVec<T> vVec(fileName.c_str());
    if ( BaseT::size() < vVec.size() ) BaseT::resize(vVec.size());
    vec<int>::const_iterator end(entries.end());
    for ( vec<int>::const_iterator itr(entries.begin()); itr != end; ++itr )
    {
  ...
  {
    BaseT::eraseIf(to_remove.begin(),to_remove.end());
    return *this;
  }
  ...
  {
    BaseT::eraseUnless(to_keep.begin(),to_keep.end());
    return *this;
  }
  ...
  {
    BaseT::eraseEntries(to_remove.begin(),to_remove.end());
    return *this;
  }
  ...
  void Sort() {
    sortInPlace(this->begin(),this->end());
  }

  // Sort this vector in place.  perm is permuted to the same sort order.
  void SortSync( vec<size_type> & perm )
  { if ( BaseT::size() > 1 )
    {
      QuickSort(0, this->size() - 1, perm);
      InsertionSort(perm);
    }
  }
  ...
  MempoolAllocator<typename T::value_type> const& getSubAllocator()
  {
    return BaseT::getSubAllocator();
  }
...
private:
  ...
  { using std::swap;
    ...
    do
    {
      ...
      while ( 1 )
      { do ++i;
        while ( i <= upper && (*this)[i] < (*this)[lower] );
        do --j;
        while ( (*this)[lower] < (*this)[j] );
        if (i > j) break;
        (*this)[i].swap((*this)[j]);
        swap(perm[i], perm[j]);
      }
  ...
  void InsertionSort(vec<size_type> & perm)
  { using std::swap;
