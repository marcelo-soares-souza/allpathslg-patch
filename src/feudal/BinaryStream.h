...
public:
  ...
  bool atEOF() {
    return mpBuf == mpEnd && !fillBuf(BUF_SIZ);
  }
  ...
  T& read( T* pT )
  {
    readVal(pT,typename Serializability<T>::type());
    return *pT;
  }
...
{
  reader.read(&pVal->first);
  reader.read(&pVal->second);
}
