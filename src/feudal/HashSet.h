...
public:
  ...
  std::allocator<X> alloc(X*) const {
    return std::allocator<X>();
  }

  T* create( size_t nTs ) const {
    return new T[nTs];
  }
  void destroy( T* pTs, size_t nTs ) const {
    delete [] pTs;
  }
};
...
public:
  ...
  const_iterator begin() const {
    return const_iterator(mBuckets,this);
  }
...
private:
  ...
  { AssertLt(mSize,capacity());
    ...
    do {
      if ( --pEmpty < mBktInfo ) pEmpty += capacity();
    }
...
public:
  ...
  factory_type const& getFactory() const {
    return mHCF;
  }
  hash_func_type const& getHasher() const {
    return mHCF;
  }
  comp_func_type const& getComparator() const {
    return mHCF;
  }

  const_iterator begin() const {
    return const_iterator(mppHHS,this);
  }
  const_iterator end() const {
    return const_iterator(mppHHS+mCapacity,this);
  }
  ...
  iterator begin() {
    return iterator(mppHHS,this);
  }
  iterator end() {
    return iterator(mppHHS+mCapacity,this);
  }
  ...
  { size_t hash = mHCF.hash(val);
    ...
    if ( pHHS->lookup(val,hash)  ) {
      pHHS->unlock();
      return false;
    }
    insert(val,hash,pHHS);
    return true;
  }
