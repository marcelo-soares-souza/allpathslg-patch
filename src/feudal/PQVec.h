...
public:
  ...
  explicit PQVecA( Alloc const& alloc ) {
    new (&allocator()) Alloc(alloc);
  }

  PQVecA( PQVecA const& that ) {
    new (&allocator()) Alloc;
    *this = that;
  }
  ...
  { byte const* buf = data();
    ...
    {
      result += nQs;
      buf += PQVecEncoder::Block::blockSize(nQs,*buf&7)-1;
    }
    return result;
  }
  ...
  PQVecA& clear()
  { byte* buf = data();
    if ( buf ) allocator().deallocate(buf,size());
    mData &= ~PTRMASK;
    return *this;
  }
  ...
  { if ( allocator()==that.allocator() )
    ...
    { byte* thisBuf = nullptr;
      ...
      if ( sz )
      {
        thisBuf = allocator().allocate(sz);
        memcpy(thisBuf,that.data(),sz);
      }
      byte* thatBuf = nullptr;
      sz = size();
      if ( sz )
      {
        thatBuf = that.allocator().allocate(sz);
        memcpy(thatBuf,data(),sz);
      }
  ...
  void readFeudal( BinaryReader& reader, size_t sz, void* )
  {
    clear();
    byte* buf = alloc(sz);
    reader.read(buf,buf+sz);
  }
  void writeFeudal( BinaryWriter& writer, void const** ) const
  {
    byte const* buf = data();
    if ( buf ) writer.write(buf,buf+size());
  }
  ...
  static unsigned fixedDataLen() {
    return 0;
  }
  static size_type interpretSize( void*, size_t sz ) {
    return sz;
  }
  ...
  { Assert(!data());
    mData |= reinterpret_cast<size_t>(buf)&PTRMASK;
    return buf;
  }
...
private:
  ...
  byte* data() {
    return reinterpret_cast<byte*>(mData&PTRMASK);
  }
  ...
  Alloc& allocator() {
    return reinterpret_cast<Alloc*>(&mData+1)[-1];
  }
...
template <class Alloc>
void swap( PQVecA<Alloc>& v1, PQVecA<Alloc>& v2 ) {
  v1.swap(v2);
}
