...
public:
  ...
  size_t getNElements() const {
    return mFCB.getNElements();
  }
  size_t getVarDataLen() const {
    return mFCB.getVarDataLen();
  }
  size_t getFixedDataLen() const {
    return mFileSize - mFCB.getFixedOffset();
  }
  FeudalControlBlock const& getFCB() const {
    return mFCB;
  }
...
void MergeMastervecs( const String& dstFile, vec<String> const& inputs )
{
  if ( inputs.size() < 2 )
  {
  ...
  size_t totVarData = 0;
  auto namesEnd(inputs.end());
  for ( auto namesItr(inputs.begin()); namesItr != namesEnd; ++namesItr )
  {
