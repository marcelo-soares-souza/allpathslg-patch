...
public:
  ...
  {
    if ( !mFwd.size() ) mFwd = mPaths.getEdgeList(mReadID);
    return mFwd;
  }
...
public:
  ...
  ReadID const& getCurReadID() const {
    return mItr->getReadID();
  }
  ...
  bool isMatchRC() const {
    return mMatchup.isRC;
  }
...
public:
  ...
  size_t getNPairs() const {
    return mLSC.getPM().nPairs();
  }
  size_t getNBatches() const {
    return (getNPairs()+BATCH_SIZE-1)/BATCH_SIZE;
  }
  size_t getNClosures() const {
