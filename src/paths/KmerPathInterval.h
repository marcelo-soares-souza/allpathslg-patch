...
inline Bool is_palindrome( kmer_id_t x ) {
  return x >= first_palindrome;
}
...
public:
  ...
  kmer_id_t Minimum( )  const {
    return(start_( ));
  }
  kmer_id_t Maximum( )  const {
    return(start_( ) + diff_( ));
  }
  int Stretch( ) const {
    return((int)(diff_()));  // Note that this is different than Length()!
  }

  Bool isGap( ) const {
    return ( (data_ & ((kmer_id_t)1<<63)) != 0 );
  }
  Bool isSeq( ) const {
    return ( (data_ & ((kmer_id_t)1<<63)) == 0 );
  }
...
private:
  ulonglong data_;
  inline kmer_id_t start_() const {
    return ( data_ & (((kmer_id_t)1<<40)-1) );
  }
  inline kmer_id_t diff_()  const {
    return ( (data_ >> 40) & 0x7fffff );
  }
