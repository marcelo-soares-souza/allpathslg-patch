    ...
    if ( G.From(s).size() > 1 ) {
      ...
      for ( size_t i = 0; i < links.size(); i++ ) {
        ...
        if ( sep2x + s2x_len - dev_mult * (s2x_dev + dev2x)
          ...
          OK = False;
          if ( VERBOSITY >= 2 ) {
            cout << "Outgoing conflict on " << s << " between ";
            PRINT2(s2,s2x);
          }
          outConflictCt++;
          outConflictLoc++;
    ...
    if ( G.To(s).size() > 1 ) {
      ...
      for ( size_t i = 0; i < links.size(); i++ ) {
        ...
        if ( sep0x + s0x_len - dev_mult * (s0x_dev + dev0x)
          ...
          OK = False;
          if ( VERBOSITY >= 2 ) {
            cout << "Incoming conflict on " << s << " between ";
            PRINT2(s0,s0x);
          }
          inConflictCt++;
          break; // one conflict is enough to stop
                             ...
                             const int MIN_LINKS_TO_PRINT )
{
  ...
  if ( VERBOSITY >= 1 ) {
    cout << "min_pair_seps: ";
    min_pair_seps.Print(cout);
    cout << endl;
    cout << "max_pair_seps: ";
    max_pair_seps.Print(cout);
    cout << endl;
  }
  ...
  for ( int break_iter = 1; break_iter <= MAX_BREAK_ITER; break_iter++ ) {
    for ( unsigned mls = 0; mls < min_links_set.size(); mls++ ) {
      ...
      for (size_t mps = 0; mps < max_pair_seps.size(); ++mps) {
        ...
        int joins = 0;
        int localJoinsCt =  0; // help track local progress
