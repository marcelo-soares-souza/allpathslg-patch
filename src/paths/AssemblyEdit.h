...
public:
  ...
  Bool Internal( ) const {
    return etype_ == INTERNAL;
  }
  Bool GapCloser( ) const {
    return etype_ == GAP_CLOSER;
  }
  ...
  const basevector& Rep( int i ) const {
    return reps_[i];
  }
  basevector& Rep( int i ) {
    return reps_[i];
  }
  const vec<basevector>& Reps( ) const {
    return reps_;
  }
