...
private:
  ...
  public:
    ...
    HyperKmerPath const& getPath() const {
      return *mpHKP;
    }

    void addAdjacency( Node* pNode ) {
      mAdjacencies.insert(pNode);
    }
    void removeAdjacency( Node* pNode ) {
      mAdjacencies.erase(pNode);
    }
    void clearAdjacencies() {
      mAdjacencies.clear();
    }
    std::set<Node*> const& getAdjacencies() const {
      return mAdjacencies;
    }
    size_t getAdjacencyCount() const {
      return mAdjacencies.size();
    }
  ...
  {
  public:
    void addHKP( HyperKmerPath const& hkp ) {
      mHKPs.insert(&hkp);
    }
    void removeHKP( HyperKmerPath const& hkp ) {
      mHKPs.erase(&hkp);
    }
  ...
  public:
    OutputAccumulator( Checkpointer& chkPtr, size_t maxMerges )
      : mChkPtr(chkPtr) {
      mToMerge.reserve(maxMerges);
    }
    virtual ~OutputAccumulator();
    void addNode( Node* pNode );
    HyperKmerPath checkpoint()
    {
      Locker locker(mChkPtr);
      return mChkPtr.checkpoint();
    }
  ...
  protected:
    ...
    vec<HyperKmerPath> const& getMerges() {
      return mToMerge;
    }
    LockedData& getLock() {
      return mLock;
