...
public:
  ...
  long getMinSep() const {
    return mMinSep;
  }
  long getMaxSep() const {
    return mMaxSep;
  }
...
public:
  ...
  PairsManager const& getPM() const {
    return mPM;
  }
  LibSeps const& getLibSeps( int libId ) const {
    return *mColl[libId];
  }
  LibSeps& getLibSeps( int libId ) {
    return *mColl[libId];
  }
  ...
  { size_t result = 0;
    LibSeps** ppSep = mColl;
    LibSeps** ppEnd = ppSep + mPM.nLibraries();
...
public:
  ...
  bvec const& getFill() const {
    return mFill;
  }
  size_t getPairId() const {
    return mPairId;
  }
...
{ if ( itr1 < itr2 ) {
      ends -= mPaths.getEdgeListLen(itr1,itr2);
    }
    if ( itr1 > itr2 ) {
      ends += mPaths.getEdgeListLen(itr2,itr1);
    }
    return ends;
  }
