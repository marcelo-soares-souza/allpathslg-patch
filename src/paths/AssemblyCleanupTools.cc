...
void Assembly::dedup( const Bool Exact ) {
  ...
  for ( int si = 0; si < scaffolds_.isize(); si++ ) {
    ...
    s2data[si].seq_ = seq;
    s2data[si].minseq_ = minseq;
    s2data[si].maxseq_ = maxseq;
    basevector seq_rc, minseq_rc, maxseq_rc;
    for ( int i = 0; i < Ni; i++ ) {
      int ci = Ni -1 -i;
      basevector ci_minseq_rc = s2altTigs[si][ci][0];
      ci_minseq_rc.ReverseComplement();
      basevector ci_seq_rc = s2altTigs[si][ci][1];
      ci_seq_rc.ReverseComplement();
      basevector ci_maxseq_rc = s2altTigs[si][ci][2];
      ci_maxseq_rc.ReverseComplement();
      ...
      if ( ci > 0 )
        for ( int pi = 0; pi < scaffolds_[si].Gap(ci-1); pi++ ) {
          seq_rc.push_back(0);
          minseq_rc.push_back(0);
          maxseq_rc.push_back(0);
        }
    }
    ForceAssertEq( minseq.size(), minseq_rc.size() );
    ForceAssertEq( seq.size(), seq_rc.size() );
    ForceAssertEq( maxseq.size(), maxseq_rc.size() );
    s2data[si].seq_rc_ = seq_rc;
    s2data[si].minseq_rc_ = minseq_rc;
    s2data[si].maxseq_rc_ = maxseq_rc;
  }
    ...
    for ( int sj = si + 1; sj < scaffolds_.isize(); sj++ ) {
      ...
      for( int pass = 0; pass < 2; pass++ ) {
        ...
        for ( int itig = 0; itig < Ni; ++itig) {
          ...
          if ( abs( (int)tigi.size() - (int)tigj.size() ) > MaxMismatch ) {
            tig_match = false;
            break;
          }
          ...
          if ( tig_size < EfastaMatchSize ) {
            ...
            if ( pass == 1 )
              for ( size_t k = 0; k < v_jbases.size(); ++k ) {
                v_jbases[k].ReverseComplement();
              }
