...
public:
  ...
  struct info_t {
    ...
    void set(const String&a,const String&b,const String&c,const String&d) {
      ID=a;
      Number=b;
      Type=c;
      Description=d;
...
//    vector<string> gt;
  ...
  size_t idx( iterator i ) {
    return i - this->begin();
  };
  size_t idx( const_iterator i ) const {
    return i - this->cbegin();
  };
  ...
  void push_back(const VCFLine& val) {
...
//     , const std::string& sColumnDelimiters="\t");
  ...
  iterator begin() {
    return _vChromosomes.begin();
  };
  iterator end() {
    return _vChromosomes.end();
  };
  const_iterator begin() const {
    return _vChromosomes.begin();
  };
  ...
  //c++11
  const_iterator cbegin() const noexcept {
    return _vChromosomes.cbegin();
  };
  const_iterator cend() const noexcept {
    return _vChromosomes.cend();
  };

  //number of chromosomes
  size_t size()const {
    return _vChromosomes.size();
  };

  //number of samples, i.e. number of tags after the FORMAT tag
  size_t getNSamples()const {
    return ((VCF_SAMPLES>_svHeader.size())?(0):(_svHeader.size()-VCF_SAMPLES));
  };

  // get line-by-lie file meta
  const vector<std::string>& getMETA()const {
    return _svMETA;
  };

  // get tag-by-tag data header
  const vector<std::string>& getHeader()const {
    return _svHeader;
  };
