...
public:
  ...
  ho_interval Interval( ) const {
    return ho_interval( pos_, Pos_ );
  }
  genome_part_id_t GenomeId( ) const {
    return genome_id_;
  }
  seq_interval SeqInterval() const {
    return seq_interval( -1, genome_id_, pos_, Pos_ );
  }
  Bool Rc( ) const {
    return rc_;
  }
  ...
  orient_t Orient() const {
    return rc_ ? ORIENT_RC : ORIENT_FW;
  }
