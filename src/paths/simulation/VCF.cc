...
ostream& operator<<(ostream&os,const VCFWriter&in) {
  for(const auto&entry:in._svMETA) {
    os<<entry<<std::endl;
  }
  for(const auto&entry:in._INFOs) {
    os<<"##INFO=<ID=" << entry.ID << ",Number="<< entry.Number<<",Type="<<entry.Type<<",Description=\""<<entry.Description<<"\">"<<std::endl;
  }
  for(const auto&entry:in._FILTERs) {
    os<<"##FILTER=<ID=" << entry.ID << ",Number="<< entry.Number<<",Type="<<entry.Type<<",Description=\""<<entry.Description<<"\">"<<std::endl;
  }
  for(const auto&entry:in._FORMATs) {
    os<<"##FORMAT=<ID=" << entry.ID << ",Number="<< entry.Number<<",Type="<<entry.Type<<",Description=\""<<entry.Description<<"\">"<<std::endl;
  }
  for(const auto&entry:in._SAMPLEs) {
    os<<"##SAMPLE=<ID=" << entry.ID << ",Description=\""<<entry.Description<<"\">"<<std::endl;
  }
  os<<"#";
