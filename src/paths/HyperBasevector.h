...
public:
  ...
  void AppendDisjointUnionOf( const vec<HyperBasevector>& v ) {
    SetToDisjointUnionOf( v, false );
  }
  ...
  int EdgeLength( int e ) const {
    return EdgeObject(e).size( );
  }
  int EdgeLengthBases( int e ) const {
    return EdgeObject(e).size( );
  }
  int Bases( int e ) const {
    return EdgeObject(e).size( );
  }

  int EdgeLengthKmers( int e ) const {
    return EdgeObject(e).isize( ) - K( ) + 1;
  }
  int Kmers( int e ) const {
    return EdgeObject(e).isize( ) - K( ) + 1;
  }
