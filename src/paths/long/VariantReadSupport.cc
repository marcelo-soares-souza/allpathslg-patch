...
void ComputeHomopolymerProb_AT_Diploid( vec<std::tuple<uint64_t,double,uint64_t>>& n_q_len) {
  ...
  for( auto& entry: n_q_len) {
    if( get<1>(entry) < minQ ) {
      ...
      max_n=max(get<0>(entry),max_n);
    }
  }
  if(max_n>3) {
    for( auto& entry: n_q_len) {
      if( get<0>(entry) == 1 ) {
        get<1>(entry)=min(get<1>(entry),0.);
      }
    }
  }
  ...
  //prevent underflow
  for( size_t ll = get<0>(n_q_len[0])+1 ; ll <= nReads ; ++ll) {
    dFactor *= ll;
  }
  for(uint64_t ff=1; ff<nFlavors; ++ff) {
    for( size_t ll = 2 ; ll <= get<0>(n_q_len[ff]) ; ++ll) {
      dFactor /= (long double)(ll);
    }
  }
  if( dFactor==std::numeric_limits<long double>::infinity() || isnan(dFactor) ) {
    dFactor=1.0;
  }
...
void AdjustHomopolymerProb(unsigned char baseval, std::unordered_map< uint64_t, vec<std::tuple<uint64_t,double,int>>>& length_lookup) {
  ...
  for(auto entry: n_q_len) {
    ...
    for( auto& var_entry: length_lookup[loc_length]) {
      get<1>(var_entry)= std::min( get<1>(var_entry), loc_q);
    }
  }
}
