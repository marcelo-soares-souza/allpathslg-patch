...
public:
  ...
  {
    ForceAssertLt(idx,mVals.size());
    return mVals[idx];
  }

  gvec& reserve( size_type nnn ) {
    mVals.reserve(nnn);
    return *this;
  }

  gvec& set( size_type idx, value_type val )
  { AssertLt(idx,mVals.size());
  ...
  { AssertLe(idx,mVals.size());
    ForceAssertLt(val,sizeof(gCharSet));
    mVals.insert(mVals.begin()+idx,val);
    return *this;
  }

  gvec& reverse()
  {
    std::reverse(mVals.begin(),mVals.end());
    return *this;
  }
  ...
  static char charFor( gvec::value_type val ) {
    return gCharSet[val];
  }
  ...
  {
    char operator()( value_type val ) const {
      return gCharSet[val];
    }
  };
...
public:
  ...
  Scorer const& getScorer() const {
    return mNW.getScorer();
  }

  bvec getConsensus() const
  { bvec result;
    result.reserve(mScores.size());
  ...
  public:
    ...
    unsigned getBestCount() const {
      return mCounts[mBestVal];
    }

    unsigned getErrCount() const {
      return nVals()-mCounts[mBestVal];
    }
    ...
    { os << gvec::charFor(mBestVal);
      char sep = '[';
      for ( gvec::value_type consVal = 0; consVal!=NVALS; ++consVal )
      {
        os << sep;
        sep = ',';
        os << eval(consVal,scorer);
      }
      os << ']';
    }
  ...
  VecConsScore const& getScores() const {
    return mScores;
  }
  ...
  { printGappedConsensus(os);
    while ( itr != end )
    {
      printAlignment(os,itr->begin(),itr->end());
      ++itr;
    }
  }
...
private:
  ...
  public:
    ...
    { using std::distance;
      ...
      mRow[0].clear();
      mRow[0].reserve(2*scores.size());
      mRow[1].clear();
      mRow[1].reserve(2*scores.size());
      firstRow(scores);
      size_t whichRow = 0;
      for ( ; itr != end; ++itr )
    ...
    { size_t consIdx = scores.size();
      ...
      { switch ( mBackPtrs[testIdx][consIdx] )
        {
        case DONE:
          AssertEq(testIdx,0ul);
          AssertEq(consIdx,0ul);
          return;
        case MATCH:
