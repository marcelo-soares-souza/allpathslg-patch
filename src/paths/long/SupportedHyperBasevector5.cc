                                    ...
                                    vec<int>& to_delete, const int verbosity )
{
  ...
  { vec<int> to_delete_loc;
    ...
    to_delete_loc.clear( ), locs1.clear( );
    locs2.clear( );
  }
}
...
//               cout << e << " --> " << support[e] << "\n";
  ...
  { if ( To(v).size( ) != 1 || From(v).size( ) != 2 ) continue;
    ...
    */
    if ( support_raw[f1] >= i_min_win ) to_delete.push_back(f2);
  } //if ( support[f1] >= min_win ) to_delete.push_back(f2);
  for ( int v = 0; v < N( ); v++ )
  { if ( To(v).size( ) != 2 || From(v).size( ) != 1 ) continue;
    ...
    */
    if ( support_raw[e1] >= i_min_win ) to_delete.push_back(e2);
  } //if ( support_raw[e1] >= min_win ) to_delete.push_back(e2);
    ...
    const vecqualvector& quals, const long_heuristics& heur, const long_logging& logc )
{
  ...
  public:
    ...
    double prob(size_t ii)const {
      return _num[ii]/_den;
    };
    const vec<double>& numerators()const {
      return _num;
    };
    const std::set<int>& activeBubbles()const {
      return _bubbles_hit;
    };
    double denominator()const {
      return _den;
    };
    uint64_t nEntries()const {
      return _nEntries;
    };
    bool logged()const {
      return _bLogged;
  ...
  public:
    ...
    read_place_buffer():_container(),_nElements(0) {
      _container.reserve(10000000);
    };
    ...
    void reserve(size_t ii) {
      _container.reserve(ii);
    };
    ...
    const read_place& front()const              {
      return _container[0];
    };
    read_place& back()                          {
      return _container[_nElements-1];
    };
    const read_place& back()const               {
      return _container[_nElements-1];
    };
    read_place& operator[](size_t ii)           {
      return _container[ii];
    };
    const read_place& operator[](size_t ii)const {
      return _container[ii];
    }

    typename vec<read_place>::iterator begin() noexcept {
      return _container.begin();
    };
    typename vec<read_place>::const_iterator begin() const noexcept {
      return _container.begin();
    };
    typename vec<read_place>::iterator end() noexcept {
      return _container.begin()+_nElements;
    };
    typename vec<read_place>::const_iterator end() const noexcept {
      return _container.begin()+_nElements;
    };
  };
