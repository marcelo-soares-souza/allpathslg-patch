...
public:
  ...
  VariantSignature(const basevector& branch1, const basevector& branch2)
    : head(), tail(),branches(2) {
    branches[0]=branch1;
    branches[1]=branch2;
  }
  const basevector& Branch1() const {
    return branches[0];
  }
  const basevector& Branch2() const {
    return branches[1];
  }
  const vec<basevector>& Branches() const {
    return branches;
  }
public:
...
public:
  ...
  { branches_.push_back(path);
    vcalls_.push_back(vcalls);
    aligns_.push_back(a);
    weights_.push_back(weight);
    trims_.push_back(trim);
  }

  void AddRefWeight(vec<double> ref_weight) {
