...
class LongProtoReadsQualsPairs {
  ...
  class path_or_data {
    ...
    load_file(U&data_,const String&path_) {
      data_.ReadAll(path_);
      return data_;
    }
    ...
    template<typename U>
    static typename std::enable_if<has_ReadAll<T>::value,U&>::type
    write_file(U&data_,const String&path_) {
      data_.WriteAll(path_);
      return data_;
    }
  ...
  public:
    ...
    path_or_data& operator=(const path_or_data&& other) {
    ...
    T& get(bool bResetFile=false) {
      return const_cast<T&>( static_cast<const path_or_data<T>&>(*this).get(bResetFile) );
    }
    ...
    void write()const {
      if(ptr && sPath.size()>0) write_file(*ptr,sPath);
    };
  private:
...
public:
  ...
  vecbasevector const& reads(bool bResetFile=false)const {
    return reads_.get(bResetFile);
  }
  vecqualvector const& quals(bool bResetFile=false)const {
    return quals_.get(bResetFile);
  }
  PairsManager  const& pairs(bool bResetFile=false)const {
    return pairs_.get(bResetFile);
  }

  vecbasevector& reads(bool bResetFile=false) {
    return reads_.get(bResetFile);
  }
  vecqualvector& quals(bool bResetFile=false) {
    return quals_.get(bResetFile);
  }
  PairsManager&  pairs(bool bResetFile=false) {
    return pairs_.get(bResetFile);
  }
  void write() {
    reads_.write();
  ...
  bool inMem()const {
    return reads_.inMem() || quals_.inMem() || pairs_.inMem();
  };
private:
...
public:
  ...
  struct key_hasher {
    size_t operator()(const key_t& in)const {
  ...
  const unit_t& operator[](const key_t& key)const {
    return this->get(key);
  };
  unit_t& operator[](const key_t& key) {
    return this->get(key);
  };
  ...
  unit_t& get(const key_t&key) {
    return const_cast<unit_t&>( static_cast<const LongProtoTmpDirManager&>(*this).get(key) );
  }
