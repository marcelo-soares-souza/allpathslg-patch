...
//                      if True in map( lambda x: x>0 and x<phred90, probs ):
            if ( any_of(probs.begin(),probs.end(),[&phred90](double x) {
            return x>0 && x<phred90;
          } )) {
//                          new_samples = samples
              new_samples = samples;
...
//                      calls = map( lambda x: x > phred995, probs )
            ...
            std::transform(probs.begin(),probs.end(),back_inserter(calls),[&phred995](double x) {
              return (unsigned int)(x>phred995);
            });
...
//                      allele1 = calls.index(True)
            size_t allele1 = find_if(calls.begin(),calls.end(),[](unsigned int x) {
              return x!=0;
            })-calls.begin();
            size_t allele2;
...
//                          allele2 = calls.index(True, allele1+1)
              allele2 = find_if(calls.begin()+allele1+1,calls.end(),[](unsigned int x) {
                return x!=0;
              })-calls.begin();
            }
...
//                      gt_idx = names.index('GT')
            size_t gt_idx = find_if(names.begin(),names.end(),[](String x) {
              return x=="GT";
            })-names.begin();
...
//                      new_samples.append( ':'.join(vals) )
            ...
            for(size_t ii=1; ii<vals.size(); ++ii) {
              new_samples.back().append(":"+vals[ii]);
            }
          }
          if(bError) break;
        }
...
//              fd.write( "\t".join( new_samples ) )
        for(const auto& entry:new_samples) {
          os << "\t" << entry;
        }
//              fd.write( "\n" )
        os<<std::endl;
