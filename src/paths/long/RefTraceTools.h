...
public:
  ...
  int   Source(int g) const          {
    return G_source[g];
  }
  bool  IsDoubled(int g) const          {
    return isDoubled[g];
  }
  const basevector& Seq(int g) const {
    return G[g];
  }
  const vecbasevector& Seqs() const  {
    return G;
  }
private:
  vec<int> G_source;
...
void EdgePlacements::AlignEdgesToRefExp(const int verbosity, ostream& out)
{
  ...
  for(size_t gg=1; gg<G.size(); ++gg) {
    max_g_len=max(max_g_len,G[gg].size());
  }

  vec<std::pair<int,int>> permutation(hbp.EdgeObjectCount());
  for(int ii=0; ii<hbp.EdgeObjectCount(); ++ii) {
    permutation[ii]=std::make_pair(hbp.EdgeObject(ii).isize(),ii);
  }
...
//              }
    ...
    #pragma omp master
    {
      const size_t n=std::accumulate(ee_vec_work.begin(),ee_vec_work.end(),size_t(0),[](size_t a,vec<work_type>const&b) {
        return a+b.size();
      });
