        ...
        { if ( start + n <= genome[g].isize( ) )
          { int64_t stop = start + n;
            ...
            if(SAMPLE=="human") {
              switch(g) {
              case 22:
                RTCtrl.getRefTags()[0]="X";
                break;
              case 23:
                RTCtrl.getRefTags()[0]="Y";
                break;
              default:
                RTCtrl.getRefTags()[0]=ToString(g+1);
                break;
              }
            }
            else {
              RTCtrl.getRefTags()[0]=ToString(g);
            }
...
public:
  ...
  { bvec const& tig = (*mpRef)[item.mRefId];
    ...
    {
      *oItr = IntTriple(KmerId(tig,mK,idx),item.mRefId,idx);
      ++oItr;
    }
  }
