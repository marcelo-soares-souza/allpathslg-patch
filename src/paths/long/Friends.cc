...
public:
  ...
  IAndOs::const_iterator fbegin() const {
    return mFriends.begin();
  }
  IAndOs::const_iterator fend() const {
    return mFriends.end();
  }
...
private:
  ...
  { OuterVec<IAndOs,MempoolAllocator<IdAndOrientation>> fVec(mAlloc);
    ...
    gMyTurn.wait(lock,
    [startId,&writer]() {
      return startId==writer.getNElements();
    });
...
private:
  ...
  void findReads( size_t nUnipaths, unsigned coverage, const int verbosity )
  {
    ...
    mReads.resize(nUnipaths);
    reserve(mReads,2*coverage);
    mRCReads.resize(nUnipaths);
    reserve(mRCReads,2*coverage);
    size_t nReads = mPaths.size();
    for ( size_t readId = 0; readId != nReads; ++readId )
                  ...
                  bool useBrokenVersion )
{
  ...
  { size_t start = std::min(reads.size(),batchNo*batchSize);
    ...
    rgf.processRange(reads,start,end,writer);
  },nThreads);
  writer.close();
}
