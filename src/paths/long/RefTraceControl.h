...
public:
  ...
  const String& getVariantOutFile()const {
    return sVariantOutFile;
  };

  const String& getRefHead() const {
    return ref_head;
  }
  const vec<String>& getRefTags()const {
    return ref_tags;
  }
  const vec<size_t>& getRefIndex()const {
    return ref_index;
  }
  const vec<size_t>& getRefStart()const {
    return ref_start;
  }
  const vec<fastavector>& getRefSeqs()const {
    return ref_seqs;
  }
  ...
  vec<size_t>& getRefIndex() {
    return ref_index;
  }
  vec<String>& getRefTags() {
    return ref_tags;
  }
  vec<fastavector>& getRefSeqs() {
    return ref_seqs;
  }
  vec<size_t>& getRefStart() {
    return ref_start;
  }
  ...
  const PairsManager& GetPM() const {
    return pm;
  }
