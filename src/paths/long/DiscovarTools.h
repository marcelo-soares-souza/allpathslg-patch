...
public:
  ...
  bool IsRefTraceOn()const {
    return bRefTraceOn;
  }
  const vec<fastavector>& getRefSeqsExt()const {
    return ref_seqs_ext;
  }
  const vec<size_t>& getRefSeqsExtExt()const {
    return ref_seqs_ext_ext;
  }
  const vec<size_t>& getRefLength()const {
    return ref_length;
  }
  const vec<size_t>& getChromIdx()const {
    return RefTraceControl::getRefIndex();
  }
