...
public:
  ...
  LinkingPairs () { };
  LinkingPairs (int nlibs_, const vec<int>& lens) {
    Init( nlibs_, lens);
  }
  ...
  const vec<int>&      GetLens( )  const {
    return tlens;
  }
  int                  GetLen ( int i ) const {
    return tlens[i];
  }

  const LibCov_t&      GetStarts ( int itig ) const {
    return starts[itig];
  }
  const LibCov_t&      GetStops ( int itig ) const {
    return stops[itig];
  }
  ...
  const StdMap<int,int>&  GetDists ( int ilib ) const {
    return dists[ilib];
  }
  const vec<bool>&     GetTigPick( int ilib) const {
    return tig_pick[ilib];
  }
  ...
  const LibLink_t&     GetLinks ( int u1, int u2 ) const {
    return GetLinks( make_pair(u1,u2) );
  }
  ...
  void AddStart( int ilib, int itig, const triple<int, int, int>& link  ) {
    starts[itig][ilib].insert(link);
  }
  ...
  void AddStop( int ilib, int itig, const triple<int, int, int>& link  ) {
    stops[itig][ilib].insert(link);
  }
  ...
  void AddDist ( int libid, int d, int u1, int u2 ) {
    dists[libid][d]++;
    tig_pick[libid][u1] = tig_pick[libid][u2] = true;
  }
