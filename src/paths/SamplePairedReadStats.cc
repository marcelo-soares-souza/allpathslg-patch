...
int main( int argc, char *argv[] ) {
  ...
  if ( VERBOSE ) {
    ...
    refInWeights.Print(cout);
    cout << endl;
    cout << "reference weights for outies:" << endl;
    refOutWeights.Print(cout);
    cout << endl;
  }
  ...
  sout.width( longestLibNameSize );
  sout.fill(' ');
  sout << libNameTag;
  sout << " ";
  short_titles.Print( sout );
  sout << endl;
  for ( size_t lib = 0; lib < nLibraries; lib++ ) {
    sout.width( libNameTag.size() );
    sout.fill(' ');
    sout << libID2libName[ lib ];
    sout << " ";
    for (unsigned it = 0; it < short_titles.size(); it++ ) {
      ...
      sout.width( short_titles[it].size() );
      sout.fill(' ');
      sout << value << " ";
    }
    sout << endl;
