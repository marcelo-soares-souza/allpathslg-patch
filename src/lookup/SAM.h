...
public:
  ...
  bool hasReferenceDictionary() const {
    return !mRefDescMap.empty();
  }
  ...
  size_t getReferenceDictionarySize() const {
    return mRefDescMap.size();
  }
  ...
  { std::vector<RefDesc> result;
    result.reserve(mRefDescMap.size());
  ...
  {
  public:
    iterator( SAMFile& file ) : mpFile(&file) {
      next();
    }
    iterator() : mpFile(0) {}
  ...
  private:
    void next() {
      if ( !mpFile->nextRecord(mRecord) ) mpFile = 0;
    }
...
public:
  ...
  uint getReadStart() const {
    return front().getReadStart();
  }

  uint getReadEnd() const {
    return back().getReadEnd();
  }

  uint getRefStart() const {
    return front().getRefStart();
  }

  uint getRefEnd() const {
    return back().getRefEnd();
  }

  /// unaligned portion of the end of the read
  uint getRightClip() const {
    return mRightClip;
  }

  /// read length including all clipping
  uint getTotalReadLen() const {
    return back().getReadEnd()+mRightClip;
  }

  /// left clip omitted from read's sequence
  uint getLeftHardClip() const {
    return mLeftHardClip;
  }

  /// right clip omitted from read's sequence.  no. of bases clipped.
  uint getRightHardClip() const {
    return mRightHardClip;
  }
  ...
  Block const* begin( uint idx = 0 ) const {
    return mBlocks + idx;
  }

  Block const* end() const {
    return mBlocks + mNBlocks;
  }

  Block const& front() const {
    return mBlocks[0];
  }

  Block const& back() const {
    return mBlocks[mNBlocks-1];
  }

  Block const& operator[]( uint idx ) const {
    return mBlocks[idx];
  }
