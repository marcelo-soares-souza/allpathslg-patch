...
public:
  ...
  int StartOnTarget() const {
    return a.StartOnTarget();
  }

  /// sets alignment start position on the target
  void SetStartOnTarget(int i)  {
    a.SetStartOnTarget(i);
  }
  /// shifts alignment start position on the target
  void AddToStartOnTarget(int i)  {
    a.AddToStartOnTarget(i);
  }
  ...
  Bool IsProper( ) const {
    return Proper( this->a, (int)this->query_length, (int)this->target_length );
  }
...
public:
  ...
  bool operator() ( const look_align &l) const {
