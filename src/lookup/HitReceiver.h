...
protected:
  ...
  bool AmbiguousRead(int query_id) const {
    return false;
  }
...
public:
  ...
  int ErrsAllowed(int query_id) const {
    return coll->MinErrors(query_id);
  }
  bool IsAmbiguous(int query_id) const {
    return coll->IsAmbiguous(query_id);
  }
