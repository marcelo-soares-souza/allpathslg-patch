...
public:
  static String filename_fix(const String& filename, const bool verbose = false ) {
    ...
    if( ext_start != String::npos) {
      base_names.push_back(filename.substr(0,ext_start));
    }
    ...
    if ( verbose ) {
      ...
      for (const auto& name : tested_names) {
        std::cout << name << "\n";
      }
      std::cout<<std::endl;
    }
...
public:
  ...
  fastavector( const String & s ) {
    assign(s.begin(),s.end());
  }

  explicit fastavector( const bvec& b ) {
    SetFrom(b);
  }
