...
size_t KmerSpectrum::sum_weighted() const
{
  const vec<size_t> & ndk = *this;     // number of distinct kmers at freq kf
  const size_t nkf = ndk.size();
  size_t sum = 0;
                           ...
                           const unsigned verbosity) const
{
  ...
  // ---- base coverage C_1 (commented out as it is only the good kmer coverage)
  // C_1 = C_K * L / ( L - K + 1)
  ...
  _coverage = (_genome_size ? _nk_total / _genome_size : 0);

  // ---- validate genome size estimate

  if (_coverage < _coverage_min) {
    _genome_size = 0;
    _genome_size_unique = 0;
    _genome_size_repetitive = 0;
    _d_SNP = 0;
