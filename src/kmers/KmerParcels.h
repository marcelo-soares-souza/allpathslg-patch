...
public:
  ...
  Validator & KmerValidator() {
    return _kmer_validator;
  }
  bool KmerValidator(const size_t kf) {
    return _kmer_validator(kf);
  }
...
public:
  ...
  void SetNumReads(const size_t n) {
    _n_reads = n;
  }
  ...
  // ---- Other
  inline void SetKeepOnDisk(const bool keep) {
    _keep_on_disk = keep;
  }
...
public:
  ...
  inline size_t GetParcelID() const {
    return _reader->GetParcelID();
  }
  inline double GetTimer() const {
    return _reader->GetTimer();
  }
