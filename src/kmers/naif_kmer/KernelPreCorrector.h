...
public:
  ...
  BaseColumn(const PC_Params & params) :
    _params(params),
    nq(4, vec<unsigned>(QUAL_MAX + 1, 0))
  {
    reset();
  }
...
public:
  ...
  explicit PreCorrector(const PreCorrector & that) :
    ...
    _blocks(),                   // no local _blocks
    _p_blocks(that._p_blocks),   // import pointer to main _blocks
    _recommends(_n_blocks),      // local _recommends
    _params(that._params),       // import _params
    _K(that._K),
    _bases(that._bases),
    _quals(that._quals),
                 ...
                 const size_t i1k)
  {
    ...
    if (true) {
      ...
      for (size_t ik = i0k; ik < i1k; ik++) {
        ...
        ForceAssertLt(krec.i_read, _bases.size());
