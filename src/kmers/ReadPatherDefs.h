...
public:
  ...
  KmerEater const& getEater() const {
    return mEater;
  }
  Dotter* getDotter() const {
    return mpDotter;
  }
...
public:
  ...
  size_t const& operator[]( size_t idx ) const {
    return mCounts[idx];
  }

  size_t getFirstTroughIndex() const {
    return mTroughIdx;
  }
  size_t getFirstPeakIndex() const {
    return mPeakIdx;
  }
  ...
  double goodFract() const {
    return 1.*weightedSum(mTroughIdx)/mTotCount;
  }
  double errFract() const {
    return 1.*weightedSum(1,mTroughIdx)/mTotCount;
  }
  size_t getNKmers() const {
    return mTotCount;
  }
...
public:
  ...
  {
    mSeq.reserve(K*dict.size()/10);
    mEdges.reserve(dict.size()/100);
  }
