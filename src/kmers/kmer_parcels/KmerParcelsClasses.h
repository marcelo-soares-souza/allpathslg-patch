...
public:
  ...
  NaifTimer & operator=(const NaifTimer & t) {
    _t = t._t;
    return *this;
  }
...
public:
  ...
  inline       uint8_t * UInt8Pointer()       {
    return _buf;
  }
  inline const uint8_t * UInt8Pointer() const {
    return _cbuf;
  }
...
public:
  ...
  // accessors
  inline uint64_t GetReadID()      const {
    return _read_ID;
  }
  inline int32_t  GetSignedPos()   const {
    return _pos;
  }
  inline size_t   GetUnsignedPos() const {
    return (_pos < 0) ? ~_pos : _pos;  // ~x = -x-1
  }
  inline size_t   GetBaseID()      const {
    return (_pos < 0) ? ~_pos : _pos;  // ~x = -x-1
  }
  ...
  // mutators
  inline KmerLoc & SetReadID(const size_t read_ID) {
    _read_ID = read_ID;
    return *this;
  }
  inline KmerLoc & SetSignedPos(const int32_t pos)   {
    _pos = pos;
    return *this;
  }

  inline KmerLoc GetRCKmerLoc() const {
    return KmerLoc(_read_ID, ~_pos);
  }
  inline KmerLoc GetRC() const {
    return GetRCKmerLoc();  // backward compatibility
  }
...
public:
  ...
  inline size_t GetK() const {
    return (_kmer.NumBits() >> 1);  // K = n_bits / 2
  }
  inline size_t GetKmerFreq()   const {
    return size();
  }
  ...
  bool IsPalindrome() const  // requires some processing...
  {
    BaseVec kmer_bv;
    kmer_bv.assignBaseBits(GetK(), _kmer.CharPointer());
    return (kmer_bv.getCanonicalForm() == CanonicalForm::PALINDROME);
  }
