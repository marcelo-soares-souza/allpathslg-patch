...
public:
  ...
  rc_iterator rcbegin() const {
    return rc_iterator(this,0u);
  }
  rc_iterator rcend() const {
    return rc_iterator(this,size());
  }

  static size_type getK() {
    return K;
  }
  ...
  {
    AssertLt(idx,size());
    return (data(idx) >> shift(idx)) & BASE_MASK;
  }
  ...
  KMer& setFront( value_type val ) {
    set(0u,val);
    return *this;
  }
  KMer& setBack( value_type val ) {
    set(K-1u,val);
    return *this;
  }
  ...
  { storage_type* oItr(mVal);
    ...
    { val = (val << BITS_PER_BASE) | mapper(*itr);
      ++itr;
  ...
  bool isFwd() const {
    return CF<K>::isFwd(begin());
  }
  bool isRev() const {
    return CF<K>::isRev(begin());
  }
  bool isPalindrome() const {
    return CF<K>::isPalindrome(begin());
  }
