...
public:
  ...
  { if ( K & 3 ) // if K is not evenly divisible by 4
    ...
    { // run algorithm byte-wise
      unsigned char* head = data_.begin();
      unsigned char* tail = head+K/4;
      while ( head != tail )
