...
public:
  ...
  int TrueLength( int id ) const {
    return true_ends_[id] - true_begins_[id];
  }

  // Returns id of super containing cg_id.
  int ToSuper( int cg_id ) const {
    return to_super_[cg_id];
  }

  // Start on super of cg_id.
  int StartOnSuper( int cg_id ) const {
    return start_on_super_[cg_id];
  }

  // Position in super of cg_id.
  int PosOnSuper( int cg_id ) const {
    return pos_on_super_[cg_id];
  }

  // Returns whole to_super map (mostly for backward compatibility).
  const vec<int> &ToSuper( ) const {
    return to_super_;
  }

  // Full map for start on supers.
  const vec<int> &StartOnSuper( ) const {
    return start_on_super_;
  }

  // Full map for position on supers.
  const vec<int> &PosOnSuper( ) const {
    return pos_on_super_;
  }

  // Returns super at ii.
  const superb &operator[]( int ii ) const {
    return supers_[ii];
  }

  // Return all supers.
  const vec<superb> &AllSupers( ) const {
    return supers_;
  }
