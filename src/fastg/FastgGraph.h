...
public:
  ...
  fastg_element& operator=(const fastg_element&in) {
    graph = in.graph;
    avgGap = in.avgGap;
    minGap = in.minGap;
    maxGap = in.maxGap;
  ...
  fastg_element& operator=(const basevector&in) {
    graph.Clear();
    addSequence(in);
    avgGap = invalidGap;
    minGap = invalidGap;
  ...
  std::tuple<int64_t,int64_t,int64_t> getGap()const {
    return std::make_tuple(avgGap,minGap,maxGap);
  };


  bool isEqual(const fastg_element&other)const;
  bool isSequence()const {
    return avgGap==invalidGap && graph.EdgeObjectCount()<2 && graph.N()==graph.EdgeObjectCount()*2;
  };
  bool isEmpty()const {
    return isSequence() && (graph.EdgeObjectCount()==0 || graph.EdgeObject(0).size()==0 );
  };
...
public:
  ...
  explicit fastg_sequence(const basevector&in) : entries(1) {
    entries.back().addSequence(in);
  } ;
  explicit fastg_sequence(const efasta&in) {
    assign(in);
  };
  fastg_sequence& operator=(const basevector&in) {
    entries.resize(1);
    entries.back() = in;
    return *this;
  } ;

  fastg_sequence& operator=(const efasta&in) {
    return assign(in);
  };
  fastg_sequence& assign(const efasta&in);
  ~fastg_sequence() { } ;

  const std::list<fastg_element>& getEntries()const {
    return entries;
  };
  std::list<fastg_element>& getEntries() {
    return entries;
  };
