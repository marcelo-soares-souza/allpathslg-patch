...
public:
  ...
  String Name() const {
    return ( id_ < 0 ? name_ : String("contig_") + ToString(id_) );
  }
  int Id() const {
    return id_;
  }
  ...
  void SetType( const contig_type type ) {
    type_ = type;
  }
  void SetRC( const bool rc ) {
    is_rc_ = rc;
  }
...
public:
  ...
  void SetBridged( const bool bridged ) {
    is_bridged_ = bridged;
  }
