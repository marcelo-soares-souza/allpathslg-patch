...
public:
  ...
  void SetTig( int i, int m ) {
    mtig_[i] = m;
  }
  void SetLen( int i, int l ) {
    len_[i] = l;
  }
  void SetGap( int i, int g ) {
    gap_[i].SetGap(g);
  }
  void SetDev( int i, int d ) {
    gap_[i].SetDev(d);
  }
  ...
  int TigPosition( const int contig_ID ) const {
    return Position( mtig_, contig_ID );
  }
