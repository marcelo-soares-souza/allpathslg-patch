...
public:
  ...
  int N( ) const {
    return from_.size( );  // number of vertices
  }
  ...
  const vec< vec<int> >& From( ) const {
    return from_;
  }
  const vec< vec<int> >& To( ) const {
    return to_;
  }
  vec< vec<int> >& FromMutable( ) {
    return from_;
  }
  vec< vec<int> >& ToMutable( ) {
    return to_;
  }

  const vec<int>& From( int v ) const {
    CheckGoodVertex(v);
    return from_[v];
  }
  const vec<int>& To( int v ) const {
    CheckGoodVertex(v);
    return to_[v];
  }
  int FromSize( int v ) const {
    CheckGoodVertex(v);
    return from_[v].size( );
  }
  int ToSize( int v ) const {
    CheckGoodVertex(v);
    return to_[v].size( );
  }

  vec<int>& FromMutable( int v ) {
    CheckGoodVertex(v);
    return from_[v];
  }
  vec<int>& ToMutable( int v ) {
    CheckGoodVertex(v);
    return to_[v];
  }
  ...
  Bool Source( int v ) const {
    return To(v).empty( );
  }
  Bool Sink( int v ) const {
    return From(v).empty( );
  }
...
public:
  ...
  const SerfVec<int>& From( int v ) const {
    CheckGoodVertex(v);
    return from_[v];
  }
  const SerfVec<int>& To( int v ) const {
    CheckGoodVertex(v);
    return to_[v];
  }
...
public:
  ...
  int IFrom( int v, int j ) const {
    return EdgeObjectIndexByIndexFrom( v, j );
  }
  ...
  int ITo( int v, int j ) const {
    return EdgeObjectIndexByIndexTo( v, j );
  }
  ...
  const vec<int>& FromEdgeObj( int v ) const {
    return from_edge_obj_[v];
  }
  const vec<int>& ToEdgeObj( int v ) const {
    return to_edge_obj_[v];
  }
  vec<int>& FromEdgeObjMutable( int v ) {
    return from_edge_obj_[v];
  }
  vec<int>& ToEdgeObjMutable( int v ) {
    return to_edge_obj_[v];
  }

  const vec< vec<int> >& FromEdgeObj( ) const {
    return from_edge_obj_;
  }
  const vec< vec<int> >& ToEdgeObj( ) const {
    return to_edge_obj_;
  }
  vec< vec<int> >& FromEdgeObjMutable( ) {
    return from_edge_obj_;
  }
  vec< vec<int> >& ToEdgeObjMutable( ) {
    return to_edge_obj_;
  }

  const vec<F>& Edges( ) const {
    return edges_;
  }
  vec<F>& EdgesMutable( ) {
    return edges_;
...
{
  DistancesToEndFunc(G,[len](F const& edge) {
    return (edge.*len)();
  },max_dist,fw,D);
}
...
public:
  ...
  int ToLeft( int e ) const {
    return to_left_[e];
  }
  int ToRight( int e ) const {
    return to_right_[e];
  }
...
public:
  ...
  bool IsFromSource( ) const {
    return D_->Source(FirstVertex());
  }
  bool IsToSink( ) const {
    return D_->Sink(LastVertex());
  }
